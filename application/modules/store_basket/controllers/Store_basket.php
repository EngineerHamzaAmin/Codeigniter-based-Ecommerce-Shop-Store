...
class Store_basket extends MX_Controller
{
    ...
    {
        //Note:this makes sure that no items on the shoppertrack have the same
        //shopper_id and session_id
        $this->load->module('store_shoppertrack');
        $original_session_id = $data['session_id'];
        $shopper_id  = $data['shopper_id'];

        $col1 = 'session_id';
        $val1 = $original_session_id;
        $col2 = 'shopper_id';
        $val2 = $shopper_id;

        $query = $this->store_shoppertrack->get_with_double_condition($col1,$val1,$col2,$val2);
        $num_rows = $query->num_rows();

        if($num_rows>0){
            session_regenerate_id();//regenrate session id to avoid conflicts
    ...
    function _fetch_the_data()
    {
        ...
        //Gather all the data to insert in the database
    ...
    {
        //Note: value type can be color/size
        if ($value_type == 'size') {
    ...
    {
        $this->load->model('mdl_store_basket');
        $query = $this->mdl_store_basket->get_with_double_condition($col1, $val1, $col2, $val2);
        return $query;
    }
