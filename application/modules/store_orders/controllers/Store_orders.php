...
class Store_orders extends MX_Controller
{
    ...
    function _send_auto_notify($update_id){
        ...
        $data['subject'] = 'Order Status Updated';
        $data['message'] = $msg;
    ...
    function submit_order_status(){
        ...
        if($submit=="Cancel"){
            ...
            $flash_msg = 'The Order Status Has Been Updated Successfully';
    ...
    function _get_mc_gross($paypal_id){
        //find out the amount taken in from this order
    ...
    function _auto_generate_order($paypal_id, $customer_session_id)
    {
        ...
        $data['order_status'] = $this->_get_mc_gross($paypal_id);
        $this->_insert($data);

        //transfers data to store_shoppertrack from store_basket
    ...
    {
        $this->load->model('mdl_store_orders');
        $query = $this->mdl_store_orders->get_with_double_condition($col1, $val1,$col2,$val2);
        return $query;
    }
